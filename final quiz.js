function startQuiz() {
  ...
  function updateQuestionAndScore() {
    const html = $(`<ul>
        <li id="js-answered"> ${STORE.currentQuestion + 1}/${STORE.questions.length}</li>
        <li id="js-score">Result: ${STORE.score}/${STORE.questions.length}</li>
      </ul>`);
  ...
  function handleSelectOption() {
    $('body').on("submit",'#js-questions', function(event) {
      ...
      if(selectedOption === currentQues.answer) {
        ...
        $(`${id}`).append(`sorry that is incorrect <br/> The correct answer is "${currentQues.answer}"<br/>`);
